...
接口测试和性能测试过程中，我们希望接口的部分入参字段能够随机取值，例如姓名、手机号码、银行卡号、身份证之类。下面是我日常使用beanshell参数化的总结。
### 前置处理器的创建步骤。
依次创建：线程组--》http取样器--》前置处理器（BeanShell PreProcessor）
### 手机号码
![](/img/20181011/bs1.png)
```
import java.util.Random;
        ...
        vars.put("mobile",number);
```

### 在http取样器调用形式如下：
![](/img/20181011/bs2.png)
...
//以下是生成客户姓名的汉字
        ...
        String firstName = chinese.charAt(new Random().nextInt(chinese.length())) + "";
        String lastName = chinese.charAt(new Random().nextInt(chinese.length())) + "";
        vars.put("username","用户"+firstName+lastName) ;
```
